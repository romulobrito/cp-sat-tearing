...
def poscheck_relatorio(med, inf, indet, tears):
    """
    Compat: mantida apenas para nao quebrar chamadas antigas (nao usada no fluxo v2).
    """
    toreada = set()
    if isinstance(tears, dict):
        for vs in tears.values():
    ...
    for v in list(indet):
        if v in toreada:
            indet.remove(v)
    inf.sort(); indet.sort()
    return med, inf, indet

def equations_kpi_bancos() -> Dict[str, List[str]]:
    """
    KPIs por bancos (doc 5.2; Eq26..Eq34) no formato estrutural.
    Variaveis ASCII (30 variaveis no total):
      - Sa, Sb, Sc, IDFa, IDFb, IDFc, PDIa, PDIb, PDIc
      - PTa_F, PTa_P, PTa_R, PTb_F, PTb_P, PTb_R, PTc_F, PTc_P, PTc_R
      - Pa, Pb, Pc
      - Parametros por banco (contabilizados como variaveis conforme documento):
        A_a, A_b, A_c, Lp_a, Lp_b, Lp_c, Dpi_a, Dpi_b, Dpi_c
    """
    return {
        # S por banco: inclui diferenca osmotica (Dpi_*) e area (A_*) como no documento
        "Eq26": ["Sa", "PTa_R", "PTa_P", "Pa", "Dpi_a", "A_a"],
        "Eq27": ["Sb", "PTb_R", "PTb_P", "Pb", "Dpi_b", "A_b"],
        "Eq28": ["Sc", "PTc_R", "PTc_P", "Pc", "Dpi_c", "A_c"],
        # IDF por banco: inclui Lp_*, Dpi_* e area A_* no termo de vazao especifica
        "Eq29": ["IDFa", "PTa_R", "PTa_P", "Pa", "Lp_a", "Dpi_a", "A_a"],
        "Eq30": ["IDFb", "PTb_R", "PTb_P", "Pb", "Lp_b", "Dpi_b", "A_b"],
        "Eq31": ["IDFc", "PTc_R", "PTc_P", "Pc", "Lp_c", "Dpi_c", "A_c"],
        # PDI por banco: diferenciais de pressao (nao dependem de parametros)
        "Eq32": ["PDIa", "PTa_F", "PTa_R"],
        "Eq33": ["PDIb", "PTb_F", "PTb_R"],
        "Eq34": ["PDIc", "PTc_F", "PTc_R"],
    }

def classify_kpi_bancos_v2(
    measured: Set[str], *,
    ...
    """
    Classifica KPIs por bancos com a mesma logica o/x/z/p/t/L (duas fases).
    enable_repair_3bc e aceito para interface uniforme (nao utilizado aqui).
    """
    eqs = equations_kpi_bancos()
...
def classify_documento_v2(
    ...
    """
    Classifica URS (balancos do documento) com a mesma logica o/x/z/p/t/L (duas fases).
    enable_repair_3bc e aceito para interface uniforme (nao utilizado aqui).
    """
...
def save_results_to_excel(
    ...
    # Ajuste: salvar sempre na pasta trabalho_atual/resultados_classificacao
    script_dir = os.path.dirname(os.path.abspath(__file__))
    base_dir = os.path.dirname(script_dir)
    results_dir = os.path.join(base_dir, "resultados_classificacao")
    os.makedirs(results_dir, exist_ok=True)
    # Se filename for caminho absoluto, manter; senao, juntar ao results_dir
...
def teste_cenarios_kpi_bancos():
    """
    Reproduz o caso de bancos do documento: executa classificacao com o bloco de KPIs de bancos.
    Por padrao, usa um conjunto base de medidas (Pa,Pb,Pc e sem pressoes de estagio),
    refletindo a nao observabilidade relatada no documento.
    """
    print(" TESTE KPI POR BANCOS (documento)")
    print("=" * 60)
    run_w_align = 1
    align_tag = "align_on" if run_w_align != 0 else "align_off"
    eqs = equations_kpi_bancos()
    ...
    base_medidas = {"Pa", "Pb", "Pc"}
    print(f"Medidas base: {sorted(base_medidas)}")

    try:
        med, inf, indet, tears_list, tears_reclassificados, tears_rebaixados, inference_eqs, tears_dict = classify_kpi_bancos_v2(
            base_medidas, W_align=run_w_align
        )
    except Exception as e:
        print(f"Erro na classificacao KPI bancos: {e}")
        return
    ...
    try:
        from datetime import datetime
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        nome_arquivo = f"resultados_KPI_Bancos_{len(eqs)}eq_{total_vars}vars_{obs_pct:.1f}pct_{align_tag}_{timestamp}.xlsx"
        save_results_to_excel(
            ...
            inference_eqs=inference_eqs,
            eqs=eqs,
            measured_vars=base_medidas,
            filename=nome_arquivo,
            inference_method=_LAST_INFERENCE_METHOD,
        )
    except Exception as e:
        print(f"Erro ao salvar Excel KPI bancos: {e}")


if __name__ == "__main__":
    # teste_cenarios_documento()
    teste_cenarios_kpi_bancos() 
